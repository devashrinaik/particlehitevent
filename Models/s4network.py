...
########################################################
# # s4network.py after moving ymodule to input 
# import torch
# import torch.nn as nn
# import os
# import sys
...
########################################################
# # s4network.py after moving ymodule to input less dynamic
# import torch
# import torch.nn as nn
# import os
# import sys
...
# class ConstrainedS4Layer(nn.Module):
#     def __init__(self, input_dim, height=13, width=21):
#         super().__init__()
#         self.height = height
#         self.width = width
#         self.hidden_dim = height * width
...
#         self.A = nn.Parameter(torch.randn(self.hidden_dim, self.hidden_dim) * 0.01)
#         self.B = nn.Parameter(torch.randn(self.input_dim, self.hidden_dim) * 0.01)
#         self.C = nn.Parameter(torch.randn(self.hidden_dim, self.hidden_dim) * 0.01)
...
#         logging.info("Initializing nearest-neighbor constraints...")
#         A_mask = torch.zeros(self.hidden_dim, self.hidden_dim)
#         B_mask = torch.zeros(self.input_dim, self.hidden_dim)
#         C_mask = torch.zeros(self.hidden_dim, self.hidden_dim)
#         peripheral_indices = []
...
# class S4PredictionNetwork(nn.Module):
#     def __init__(self, input_dim, hidden_output_dim, final_output_dim, height=13, width=21):
#         super().__init__()
#         logging.info(f"Initializing S4PredictionNetwork with hidden_output_dim={hidden_output_dim}, final_output_dim={final_output_dim}")
#         self.s4_layer = ConstrainedS4Layer(input_dim, height, width)
#         s4_output_dim = self.s4_layer.peripheral_indices.shape[0]
...
# Models/s4network.py after accommodating new input/target dimensions
import torch
import torch.nn as nn
import os
import sys
...
class ConstrainedS4Layer(nn.Module):
    # Added input_actual_dim to handle the true input features (e.g., 274)
    def __init__(self, input_actual_dim, height=13, width=21):
        super().__init__()
        self.input_actual_dim = input_actual_dim
        self.height = height
        self.width = width
        self.hidden_dim = height * width # This is 273, the internal dimension for S4 processing
        
        logging.info(f"Creating S4 layer with actual input_dim={self.input_actual_dim}, grid {height}x{width} ({self.hidden_dim} S4 hidden units)")

        # Projection layer to map from actual input dimension to S4 hidden dimension
        # This will map, for example, 274 features to 273 features.
        self.input_projection = nn.Linear(self.input_actual_dim, self.hidden_dim)

        # S4 specific matrices (A, B, C) operate on self.hidden_dim (273)
        self.A = nn.Parameter(torch.randn(self.hidden_dim, self.hidden_dim) * 0.01)
        # B matrix now relates the projected input (hidden_dim) to the state (hidden_dim)
        self.B = nn.Parameter(torch.randn(self.hidden_dim, self.hidden_dim) * 0.01) 
        self.C = nn.Parameter(torch.randn(self.hidden_dim, self.hidden_dim) * 0.01)
        ...
        self.register_buffer("A_mask", A_mask)
        self.register_buffer("B_mask", B_mask) # B_mask is diagonal, applied to the (hidden_dim, hidden_dim) B
        self.register_buffer("C_mask", C_mask)
        self.register_buffer("peripheral_indices", peripheral_indices)
        logging.info(f"Peripheral indices count: {peripheral_indices.shape[0]}")

    def _initialize_nearest_neighbor_constraints(self):
        logging.info("Initializing nearest-neighbor constraints for S4 hidden dimension...")
        # Masks are based on self.hidden_dim (273)
        A_mask = torch.zeros(self.hidden_dim, self.hidden_dim)
        B_mask = torch.zeros(self.hidden_dim, self.hidden_dim) # This mask is for the (hidden_dim, hidden_dim) B matrix
        C_mask = torch.zeros(self.hidden_dim, self.hidden_dim)
        peripheral_indices = []
        ...
        for i in range(self.height):
            for j in range(self.width):
                ...
                neighbors = [n for n in neighbors if n is not None]
                A_mask[idx, neighbors] = 1
                B_mask[idx, idx] = 1 # B_mask is diagonal
                if i in {0, self.height - 1} or j in {0, self.width - 1}:
                    C_mask[idx, idx] = 1
                    peripheral_indices.append(idx)

        logging.info(f"Mask init complete for S4 hidden_dim. Peripheral count: {len(peripheral_indices)}")
        return A_mask, B_mask, C_mask, torch.tensor(peripheral_indices)

    def forward(self, x):
        # x input shape: (batch_size, seq_len, self.input_actual_dim) e.g. (B, 80, 274)
        logging.debug(f"S4Layer raw input shape: {x.shape}")

        # Project input features from actual_dim to hidden_dim for S4 processing
        # (B, Seq, ActualIn) -> (B, Seq, Hidden) e.g. (B, 80, 274) -> (B, 80, 273)
        projected_x = self.input_projection(x)
        logging.debug(f"S4Layer projected_x shape: {projected_x.shape}")

        masked_A = self.A * self.A_mask
        masked_B = self.B * self.B_mask # masked_B is (hidden_dim, hidden_dim)

        batch_size, seq_len, _ = projected_x.shape # Using projected_x for sequence processing
        state = torch.zeros(batch_size, self.hidden_dim, device=projected_x.device)

        for t in range(seq_len):
            u_t = projected_x[:, t, :] # u_t is now (batch_size, self.hidden_dim) e.g. (B, 273)
            # State update: state @ A.T + u_t @ B.T (all dimensions match hidden_dim)
            state = torch.tanh(state @ masked_A.T + u_t @ masked_B.T)
            if t == 0 or t == seq_len - 1:
                logging.debug(f"Step {t}: state.mean() = {state.mean().item():.4f}")

        # Output is based on peripheral indices of the S4 state
        y_t = state[:, self.peripheral_indices]
        logging.debug(f"S4Layer output (y_t) shape: {y_t.shape}")
        return y_t
    ...
    def save_matrices(self, output_dir="results/matrices"):
        ...
        torch.save(self.B.detach().cpu(), os.path.join(output_dir, "B_matrix.pt"))
        torch.save(self.C.detach().cpu(), os.path.join(output_dir, "C_matrix.pt"))
        # Optionally save the input_projection layer's weights
        torch.save(self.input_projection.state_dict(), os.path.join(output_dir, "input_projection_state_dict.pt"))
        logging.info(f"Matrices and input_projection saved to {output_dir}")

class S4PredictionNetwork(nn.Module):
    # Added input_dim (actual input features, e.g. 274)
    # final_output_dim is the new target dimension (e.g. 8)
    def __init__(self, input_dim, hidden_output_dim, final_output_dim, height=13, width=21):
        super().__init__()
        logging.info(f"Initializing S4PredictionNetwork with input_dim={input_dim}, hidden_output_dim={hidden_output_dim}, final_output_dim={final_output_dim}")
        
        # Pass the actual input_dim to ConstrainedS4Layer
        self.s4_layer = ConstrainedS4Layer(input_actual_dim=input_dim, height=height, width=width)
        
        # s4_output_dim is determined by peripheral_indices count from S4 layer, which is unchanged
        s4_output_dim = self.s4_layer.peripheral_indices.shape[0]

        self.fc1 = nn.Linear(s4_output_dim, hidden_output_dim)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(0.2)
        # fc2 outputs the final target dimension (e.g. 8)
        self.fc2 = nn.Linear(hidden_output_dim, final_output_dim)

    def forward(self, x):
        # x input shape: (batch_size, seq_len, input_dim) e.g. (B, 80, 274)
        logging.debug(f"PredictionNetwork input shape: {x.shape}")
        s4_output = self.s4_layer(x) # s4_output shape: (batch_size, s4_output_dim)
        hidden = self.relu(self.fc1(s4_output))
        hidden = self.dropout(hidden)
        output = self.fc2(hidden) # output shape: (batch_size, final_output_dim) e.g. (B, 8)
        logging.debug(f"Prediction output shape: {output.shape}")
        return output